class UsErrorsController < ApplicationController
  ...
  def render_custom_error(status=500)
    ...
    respond_to do |format|
      format.html { use_layout ? render(template: 'errors/error_status', layout: false, status: status) : render(template: 'errors/error_status_no_layout', layout: false, status: status) }
